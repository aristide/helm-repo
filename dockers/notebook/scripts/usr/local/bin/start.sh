...
# the d4now user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ]; then
    ...
    # Refit the d4now user to the desired the user (NB_USER)
    if id d4now &> /dev/null; then
        if ! usermod --home "/home/${NB_USER}${DOMAIN:+@$DOMAIN}" --login "${NB_USER}" d4now 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the d4now user:"
            _log "- username: d4now       -> ${NB_USER}"
            _log "- home dir: /home/d4now -> /home/${NB_USER}${DOMAIN:+@$DOMAIN}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the d4now user nor '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    if [ "${NB_UID}" = "$(id -u d4now 2>/dev/null)" ]; then
        NB_UID=$(id -u "${NB_USER}" 2>/dev/null)
        NB_UID=${NB_UID:-$(id -u d4now 2>/dev/null)}
    fi
    if [ "${NB_GID}" = "$(id -g d4now 2>/dev/null)" ]; then
        NB_GID=$(id -g "${NB_USER}" 2>/dev/null)
        NB_GID=${NB_GID:-$(id -g d4now 2>/dev/null)}
    fi
    ...
    if [ "${NB_UID}" != "$(id -u "${NB_USER}")" ] || [ "${NB_GID}" != "$(id -g "${NB_USER}")" ]; then
        ...
        # Otherwise, and if not the d4now user, add it to the default group
        if [[ "${NB_USER}" != "d4now" ]]; then
    ...
    # Move or symlink the d4now home directory to the desired user's home
    ...
    if [[ "${NB_USER}" != "d4now" ]]; then
        if [[ ! -e "/home/${NB_USER}${DOMAIN:+@$DOMAIN}" ]]; then
            _log "Attempting to copy /home/d4now to /home/${NB_USER}${DOMAIN:+@$DOMAIN}..."
            mkdir "/home/${NB_USER}${DOMAIN:+@$DOMAIN}"
            # shellcheck disable=SC2086
            if cp ${CP_OPTS:--a} /home/d4now/. "/home/${NB_USER}${DOMAIN:+@$DOMAIN}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/d4now to /home/${NB_USER}${DOMAIN:+@$DOMAIN}!"
                _log "Attempting to symlink /home/d4now to /home/${NB_USER}${DOMAIN:+@$DOMAIN}..."
                if ln -s /home/d4now "/home/${NB_USER}${DOMAIN:+@$DOMAIN}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/d4now to /home/${NB_USER}${DOMAIN:+@$DOMAIN} or to create symlink!"
                    exit 1
                fi
            fi
        # The home directory could be bind mounted. Populate it.
            ...
            fd="$(comm -13 <(cd  "/home/${NB_USER}${DOMAIN:+@$DOMAIN}"; ls -A) <(cd /home/d4now; ls -A) \
                | paste -sd ',' -)"
            ...
            if [ -z "${fd}" ]; then
                ...
                _log "- ${fd}"
                if eval "cp ${CP_OPTS:--a} /home/d4now/${sf} /home/${NB_USER}${DOMAIN:+@$DOMAIN}"; then
                    date -uIseconds > "/home/${NB_USER}${DOMAIN:+@$DOMAIN}/.populated"
                    _log "Done populating home dir"
                else
                    _log "ERROR: Failed to copy data from /home/d4now to /home/${NB_USER}${DOMAIN:+@$DOMAIN}!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/d4now/"* ]]; then
...
# user we started as.
    ...
    D4NOW_UID="$(id -u d4now 2>/dev/null)"  # The default UID for the d4now user
    D4NOW_GID="$(id -g d4now 2>/dev/null)"  # The default GID for the d4now user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old d4now user to nayvoj ($(id -u d4now):$(id -g d4now))"
            ...
            sed --expression="s/^d4now:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/d4now:$(which zsh)" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "d4now" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/d4now. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/d4now ]]; then
        _log "WARNING: no write access to /home/d4now. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
